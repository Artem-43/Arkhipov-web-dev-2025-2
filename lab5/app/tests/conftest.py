...
from app.repositories.visit_logs_repository import VisitLogRepository
import os
...
def example_role(db_connector):
    ...
    # Удалить всех пользователей, которые ссылаются на эту роль (если остались)
    with connection.cursor() as cursor:
...
def existing_user(db_connector, example_role):
    ...
    # Удалить все visit_logs, связанные с этим пользователем
    with connection.cursor() as cursor:
        cursor.execute('DELETE FROM visit_logs WHERE user_id = %s;', (user.id,))
...
@pytest.fixture
def visit_logs_repository(db_connector):
    return VisitLogRepository(db_connector)


@pytest.fixture
def example_role_for_logs(db_connector):
    Role = namedtuple('Role', ['id', 'name'])
    user_role = Role(1, 'user')
    admin_role = Role(2, 'admin')

    connection = db_connector.connect()
    with connection.cursor() as cursor:
        cursor.execute('INSERT INTO roles(id, name) VALUES (%s, %s);', (user_role.id, user_role.name))
        cursor.execute('INSERT INTO roles(id, name) VALUES (%s, %s);', (admin_role.id, admin_role.name))
        connection.commit()
    ...
    with connection.cursor() as cursor:
        cursor.execute('DELETE FROM users WHERE role_id IN (%s, %s);', (user_role.id, admin_role.id))
        cursor.execute('DELETE FROM roles WHERE id IN (%s, %s);', (user_role.id, admin_role.id))
        connection.commit()


@pytest.fixture
def existing_user_for_logs(db_connector, example_role_for_logs):
    User = namedtuple('User', ['id', 'username', 'password', 'first_name', 'middle_name', 'last_name', 'role_id'])
    ...
    user_data = User(
        ...
        middle_name="Иванович",
        last_name="Иванов",
        role_id=example_role_for_logs['user'].id
    )
    ...
    admin_data = User(
        ...
        last_name="Админов",
        role_id=example_role_for_logs['admin'].id
    )
    ...
    with connection.cursor() as cursor:
        ...
        VALUES (%s, %s, SHA2(%s, 256), %s, %s, %s, %s)
        """
        cursor.execute(query, (user_data.id, user_data.username, user_data.password, user_data.first_name, user_data.middle_name, user_data.last_name, user_data.role_id))
        cursor.execute(query, (admin_data.id, admin_data.username, admin_data.password, admin_data.first_name, admin_data.middle_name, admin_data.last_name, admin_data.role_id))
        connection.commit()
    ...
    with connection.cursor() as cursor:
        cursor.execute('DELETE FROM visit_logs WHERE user_id IN (%s, %s);', (user_data.id, admin_data.id))
        cursor.execute('DELETE FROM users WHERE id IN (%s, %s);', (user_data.id, admin_data.id))
        cursor.execute("DELETE FROM visit_logs;")
        connection.commit()

@pytest.fixture
def clear_visit_logs(visit_logs_repository):
